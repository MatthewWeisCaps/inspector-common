...
#
# This script will not override any preexisting JSON.scala or MsgPack.scala files in its output directory and will make
# a best-attempt effort to validate its placement in the project structure (based on folder conventions) before running.

# check for conflicting files
if [ -f 'JSON.scala' ]; then
  echo 'ERROR: a file named JSON.scala already exists in the directory';
  exit 1
fi
if [ -f 'MsgPack.scala' ]; then
  echo 'ERROR: a file named MsgPack.scala already exists in the directory';
  exit 1
fi
